...
@Component({ selector: 'movies-ui-spinner', template: '' })
...
describe('MovieEditComponent', () => {
   ...
   *
   */
  it('should create component instance for creating new movie', (done) => {
    movieServiceSpy.read.and.returnValue(of(undefined))
    ...
    expect(component.movie).toEqual(new Movie({}))
    expect(component.title).toEqual('Nieuwe film')
    ...
    fixture
      ...
      .then(() => {
        ...
        sub.unsubscribe()
        expect(movieServiceSpy.read).not.toHaveBeenCalled()
        done()
      })
      .catch(fail)
  })
